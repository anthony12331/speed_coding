...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // 5 TRIVIA QUESTIONS VALIDATOR (ACCEPTS ANY OF THE 5 OFFICIAL ANSWERS FOR HIGH CONCURRENCY)
  const ALL_ACCEPTED_ANSWERS = [
    // 1. President
  ...
  function validateStudentAnswer(input) {
    const clean = input.trim().toLowerCase().replace(/\s+/g, ' ').replace(/[\.\,]/g, '');
    if (clean.includes('misamis oriental institute of science and technology')) return true;
    return ALL_ACCEPTED_ANSWERS.includes(clean);
  }
  ...
  if (claimBtn) {
    claimBtn.addEventListener('click', async () => {
      ...
      if (!validateStudentAnswer(inputVal)) {
        showResult('error', 'Incorrect answer! Please check your spelling and try answering any of the 5 questions listed above.');
        return;
      }
